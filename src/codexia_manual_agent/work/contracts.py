...
def _validate_timestamp(value: Any, field_name: str) -> str:
    if (
        ...
        raise InvalidWorkRecordError(f"{field_name} must be bounded canonical ISO-8601")
    try:
        parsed = datetime.fromisoformat(value)
    except (TypeError, ValueError) as exc:
        raise InvalidWorkRecordError(f"{field_name} must be canonical ISO-8601") from exc
...
def _bounded_text(value: Any, field_name: str, max_chars: int) -> str:
    ...
    if not normalized or len(normalized) > max_chars or "\x00" in normalized:
        raise InvalidWorkRecordError(f"{field_name} is empty or exceeds its text budget")
    return normalized
...
class WorkStatement:
    """Attributed text whose author kind cannot be erased during orchestration."""
    ...
    def create(
        ...
        *,
        ...
        normalized_kind = _normalize_actor_kind(author_kind)
        normalized_actor = _bounded_text(actor, "actor", MAX_ACTOR_CHARS)
        normalized_text = _bounded_text(text, "text", MAX_WORK_TEXT_CHARS)
        base = {
            "schema_version": WORK_STATEMENT_SCHEMA_VERSION,
            "statement_id": statement_id,
            "created_at": created_at,
            "author_kind": normalized_kind.value,
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "actor", _bounded_text(self.actor, "actor", MAX_ACTOR_CHARS))
        object.__setattr__(self, "text", _bounded_text(self.text, "text", MAX_WORK_TEXT_CHARS))
...
class WorkResourceRef:
    """A context locator only; referencing a resource grants no access or authority."""
    ...
    def create(
        ...
        *,
        ...
        normalized_locator = _bounded_text(locator, "locator", MAX_RESOURCE_LOCATOR_CHARS)
        normalized_label = _optional_bounded_text(label, "label", MAX_RESOURCE_LABEL_CHARS)
        base = {
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "locator",
            _bounded_text(self.locator, "locator", MAX_RESOURCE_LOCATOR_CHARS),
        )
        object.__setattr__(
            self,
            "label",
            _optional_bounded_text(self.label, "label", MAX_RESOURCE_LABEL_CHARS),
        )
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "WorkResourceRef":
        value = _exact_keys(
            payload,
            {"schema_version", "resource_id", "kind", "locator", "label", "resource_digest"},
            "WorkResourceRef",
        )
        return cls(
            schema_version=value["schema_version"],
...
class WorkIntentInterpretation:
    """A bounded interpretation of human work intent, never human-authored by proxy."""
    ...
    def create(
        ...
        *,
        ...
        normalized_kind = _normalize_actor_kind(interpreter_kind)
        if normalized_kind is WorkActorKind.HUMAN:
            raise InvalidWorkRecordError(
                "Intent interpretation cannot impersonate a human source"
            )
        statements = tuple(basis_statements)
        if not statements or len(statements) > MAX_INTERPRETATION_BASIS:
            raise InvalidWorkRecordError(
                "Intent interpretation requires a bounded non-empty statement basis"
            )
        if any(not isinstance(item, WorkStatement) for item in statements):
            raise InvalidWorkRecordError(
                "Intent interpretation basis must contain WorkStatement records"
            )
        digests = tuple(item.statement_digest for item in statements)
        ...
        created_at = created_at or _new_timestamp()
        normalized_interpreter = _bounded_text(interpreter, "interpreter", MAX_ACTOR_CHARS)
        completion = _bounded_text(
            completion_expectation,
            "completion_expectation",
            MAX_WORK_TEXT_CHARS,
        )
        continuation = _bounded_text(
            continuation_scope,
            "continuation_scope",
            MAX_WORK_TEXT_CHARS,
        )
        ...
        base = {
            ...
            "created_at": created_at,
            "interpreter_kind": normalized_kind.value,
            "interpreter": normalized_interpreter,
            "basis_statement_digests": list(digests),
            "completion_expectation": completion,
            "continuation_scope": continuation,
            "depth_interpretation": depth,
        }
        return cls(
            schema_version=WORK_INTENT_INTERPRETATION_SCHEMA_VERSION,
            interpretation_id=interpretation_id,
            created_at=created_at,
            interpreter_kind=normalized_kind,
            interpreter=normalized_interpreter,
    ...
    def __post_init__(self) -> None:
        if self.schema_version != WORK_INTENT_INTERPRETATION_SCHEMA_VERSION:
            raise InvalidWorkRecordError("Unsupported work intent interpretation schema")
        ...
        if len(set(self.basis_statement_digests)) != len(self.basis_statement_digests):
            raise InvalidWorkRecordError(
        ...
        object.__setattr__(
            self,
            "continuation_scope",
            _bounded_text(self.continuation_scope, "continuation_scope", MAX_WORK_TEXT_CHARS),
        )
        object.__setattr__(
            self,
            "depth_interpretation",
    ...
    def to_dict(self) -> dict[str, Any]:
        return {**self._base_dict(), "interpretation_digest": self.interpretation_digest}
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "WorkIntentInterpretation":
        ...
        digests_raw = value["basis_statement_digests"]
        ...
        return cls(
            ...
            basis_statement_digests=tuple(digests_raw),
...
@dataclass(frozen=True, slots=True)
class WorkHandoff:
    """An exact human delegation of work intent; it is not an authority grant."""
    ...
    intent_interpretation: WorkIntentInterpretation
    ...
    def create(
        ...
        *,
        ...
        if not isinstance(intent_interpretation, WorkIntentInterpretation):
            raise InvalidWorkRecordError(
                "intent_interpretation must be a WorkIntentInterpretation"
            )
        context_tuple = tuple(context)
        constraint_tuple = tuple(human_constraints)
        resource_tuple = tuple(resources)
        attention_tuple = tuple(attention_constraints)
        cls._validate_members(
            objective=objective,
            context=context_tuple,
            human_constraints=constraint_tuple,
            resources=resource_tuple,
            plan_resource_id=plan_resource_id,
            intent_interpretation=intent_interpretation,
            attention_constraints=attention_tuple,
        )
        ...
        base = {
            ...
            "context": [item.to_dict() for item in context_tuple],
            "human_constraints": [item.to_dict() for item in constraint_tuple],
            "resources": [item.to_dict() for item in resource_tuple],
            "plan_resource_id": plan_resource_id,
            "intent_interpretation": intent_interpretation.to_dict(),
            "attention_constraints": [item.to_dict() for item in attention_tuple],
        }
        return cls(
            schema_version=WORK_HANDOFF_SCHEMA_VERSION,
            handoff_id=handoff_id,
    ...
    def _validate_members(
        *,
        ...
        all_statements = (objective, *context, *human_constraints, *attention_constraints)
        ...
        if any(item.author_kind is not WorkActorKind.HUMAN for item in human_constraints):
            raise InvalidWorkRecordError(
                "human_constraints must preserve explicit human authorship"
            )
        if any(
            item.author_kind is not WorkActorKind.HUMAN for item in attention_constraints
        ):
            raise InvalidWorkRecordError(
        ...
        statement_digests = {item.statement_digest for item in all_statements}
        if objective.statement_digest not in intent_interpretation.basis_statement_digests:
            raise InvalidWorkRecordError(
                "Intent interpretation must bind the exact human objective"
            )
        if not set(intent_interpretation.basis_statement_digests).issubset(
            statement_digests
        ):
            raise InvalidWorkRecordError(
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.objective, WorkStatement):
            raise InvalidWorkRecordError("objective must be a WorkStatement")
        if self.objective.author_kind is not WorkActorKind.HUMAN:
            raise InvalidWorkRecordError(
                "A WorkHandoff objective must preserve explicit human authorship"
            )
        ...
        if not isinstance(self.intent_interpretation, WorkIntentInterpretation):
            raise InvalidWorkRecordError(
                "intent_interpretation must be a WorkIntentInterpretation"
            )
        ...
        self._validate_members(
            ...
            intent_interpretation=self.intent_interpretation,
    ...
    def _base_dict(self) -> dict[str, Any]:
        return {
            ...
            "human_constraints": [item.to_dict() for item in self.human_constraints],
            "resources": [item.to_dict() for item in self.resources],
            "plan_resource_id": self.plan_resource_id,
            "intent_interpretation": self.intent_interpretation.to_dict(),
            "attention_constraints": [item.to_dict() for item in self.attention_constraints],
        }
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "WorkHandoff":
        ...
        return cls(
            ...
            context=tuple(WorkStatement.from_dict(item) for item in value["context"]),
            human_constraints=tuple(
                WorkStatement.from_dict(item) for item in value["human_constraints"]
            ),
            resources=tuple(
                WorkResourceRef.from_dict(item) for item in value["resources"]
            ),
            plan_resource_id=value["plan_resource_id"],
            intent_interpretation=WorkIntentInterpretation.from_dict(
                value["intent_interpretation"]
            ),
            attention_constraints=tuple(
                WorkStatement.from_dict(item) for item in value["attention_constraints"]
            ),
...
class AttentionAssessment:
    """Dynamic cognition about human attention; it grants no execution authority."""
    ...
    def create(
        ...
        *,
        ...
        normalized_kind = _normalize_actor_kind(assessor_kind)
        if normalized_kind is WorkActorKind.HUMAN:
            raise InvalidWorkRecordError(
        ...
        normalized_urgency = _normalize_attention_urgency(urgency)
        normalized_assessor = _bounded_text(assessor, "assessor", MAX_ACTOR_CHARS)
        normalized_reason = _bounded_text(reason, "reason", MAX_ATTENTION_REASON_CHARS)
        ...
        cls._validate_decision_shape(
            ...
            requested_response=normalized_response,
        )
        base = {
            ...
            "assessor_kind": normalized_kind.value,
            "assessor": normalized_assessor,
            "needs_human": needs_human,
            "confidence_basis_points": confidence_basis_points,
            "urgency": normalized_urgency.value,
    ...
    def to_dict(self) -> dict[str, Any]:
        return {**self._base_dict(), "assessment_digest": self.assessment_digest}
