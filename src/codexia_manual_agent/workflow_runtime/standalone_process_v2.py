...
class StandaloneProcessCompletionCriterionV2:
    ...
    def evaluate(
        ...
        if claim.artifact_refs:
            return CompletionCriterionResult(
                accepted=False,
                reason="Standalone process completion does not use ArtifactRef basis.",
            )
        if claim.child_completion_refs:
            return CompletionCriterionResult(
                accepted=False,
                reason="Standalone process completion does not use child completion basis.",
            )
        if len(claim.evidence_refs) != 1:
            return CompletionCriterionResult(
                accepted=False,
                reason="Standalone process completion requires one outcome EvidenceRef.",
            )
        ...
        if evidence.locator != process_outcome_evidence_locator(
            ...
            outcome_id=evidence.evidence_id,
        ):
            return CompletionCriterionResult(
                accepted=False,
                reason="Standalone process completion evidence locator is not exact.",
            )

        return CompletionCriterionResult(
            accepted=True,
            reason="Standalone process completion basis is accepted.",
        )
