...
RECOVERY_PROGRAM = textwrap.dedent(
    ...
    print(json.dumps({
        ...
        "baseline_seeds": [item.seed for item in result.baseline_evidence],
        "candidate_seeds": [item.seed for item in result.candidate_evidence],
        "baseline_receipts": [item.physical_receipt_digest for item in result.baseline_evidence],
        "candidate_receipts": [item.physical_receipt_digest for item in result.candidate_evidence],
    }, sort_keys=True, separators=(",", ":")))
    """
).strip()


class M443ComparisonFalsificationClosureTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.db = self.root / "m4-4-3.sqlite3"
    ...
    def _session(self) -> str:
        ...
        self.m3.start_session(
            ...
            payload={
                ...
                "prompt_version": "m4.4.3-falsification-closure",
    ...
    def test_real_frozen_comparison_refutes_claim_and_recovers_without_rerun(self) -> None:
        ...
        self.assertEqual(tuple(item.seed for item in result.baseline_evidence), seeds)
        self.assertEqual(tuple(item.seed for item in result.candidate_evidence), seeds)
        ...
        self.assertEqual(recovered["baseline_mean"], "184")
        self.assertEqual(recovered["candidate_mean"], "8")
        self.assertEqual(recovered["effect"], "176")
        self.assertEqual(tuple(recovered["baseline_seeds"]), seeds)
        self.assertEqual(tuple(recovered["candidate_seeds"]), seeds)

        expected_baseline_receipts = tuple(
            physical_before[run.run_id][3] for run in baseline_runs
        )
        expected_candidate_receipts = tuple(
            physical_before[run.run_id][3] for run in candidate_runs
        )
        self.assertEqual(
            tuple(recovered["baseline_receipts"]),
            expected_baseline_receipts,
        )
        self.assertEqual(
            tuple(recovered["candidate_receipts"]),
            expected_candidate_receipts,
        )

        for path, before_bytes, before_mtime, _receipt_digest in physical_before.values():
