...
def _attention(snapshot, proposal, admission, *, ask_human: bool = False):
    ...
    return DynamicAttentionDecision.evaluate(
        handoff=snapshot.handoff,
        interpretation=snapshot.interpretation,
        proposal=proposal,
        admission=admission,
...
def _revision_checkpoint(snapshot):
    proposal = ContinuationProposal.create(
        ...
        statement=_statement(
            WorkActorKind.WORKER,
            "chatgpt",
            "Use a disposable shortcut and treat the unsupported result as complete.",
        ),
    )
    admission = ContinuationAdmission.evaluate(
        handoff=snapshot.handoff,
        ...
        reason="The worker candidate stays in scope but is too shallow and unsupported.",
        revision_request="Use the required depth and verify the result before continuing.",
    )
...
def test_driver_sends_worker_revision_then_continues_without_human_turn(tmp_path) -> None:
    supervisor, snapshot, peer, client = _registered(tmp_path)
    driver = BackgroundWorkDriver(supervisor)
    ...
    assert result.provider_turns == 2
    assert len(client.send_calls) == 2
    first_prompt = client.send_calls[0][1]
    assert "[Codexia delegated-work revision]" in first_prompt
    assert "Use the required depth and verify the result" in first_prompt
    second_prompt = client.send_calls[1][1]
    assert "[Codexia delegated-work continuation]" in second_prompt


def test_driver_reconciles_revision_after_crash_without_resend(tmp_path) -> None:
    supervisor, snapshot, peer, client = _registered(tmp_path)
    proposal, admission, attention = _revision_checkpoint(snapshot)
    prepared = supervisor.record_checkpoint(
        snapshot.work_id,
        proposal=proposal,
        admission=admission,
    ...
    dispatch = in_flight.pending_dispatch
    assert dispatch is not None
    prompt = ChatGPTPeerLoop._render_codexia_revision(
        handoff=in_flight.handoff,
        interpretation=in_flight.interpretation,
        proposal=dispatch.proposal,
        admission=dispatch.admission,
