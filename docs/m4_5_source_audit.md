...
| Recovery silently changes the comparison criterion | M4.5.3 receives only the SQLite path plus the existing stable `policy_id` and recovers the already-frozen policy/result; no replacement policy creation path exists in the child process. |
...
A fresh Python process receives only the SQLite path and existing stable `policy_id`. It independently reconstructs the policy, result, conclusion identity, and bounded conclusion through recovery registries. It has no experiment runner, authorization authority, replacement policy construction, or caller-provided conclusion text.

The parent verifies the child-recovered conclusion id/digest against the pre-restart publication and verifies all four governed physical result files have unchanged bytes and `mtime_ns` after the child exits.
