...
| Stale completion evidence | older worker result cannot close work after newer durable human/external activity |
...
| Pilot CLI smoke | `start / drive / answer / status` parse through the experimental CLI surface |
...
- Codexia decides completion only from terminal exact worker evidence followed by the final live-peer reread;
