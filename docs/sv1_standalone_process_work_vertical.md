...
SV1 is the first end-to-end host composition proof over the Gen2 semantic
Core after G2.41 exposed durable capability state to completion criteria.
...
The host records that EvidenceRef in Work chronology. Before creating Work, the
standalone composition also requires the provider distribution to match the
canonical PackBinding 2.0.0 exactly, including pack identity, version,
definition digest, and members.
...
The Pack-defined completion criterion requires the exact SV1 claim shape and
binds it to durable G2.41 capability context:

- exact Pack 2.0.0 semantics;
- exact completion summary;
- no ArtifactRef basis;
- no child WorkCompletion basis;
- exactly one process CapabilityNeed;
- exact CapabilityBinding / operation / parameters;
- durable SUCCEEDED CapabilityOutcome;
- exactly one process-success EvidenceRef whose identity and digest match that
